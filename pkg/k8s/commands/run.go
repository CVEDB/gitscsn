...
import (
	...
	cmd "github.com/aquasecurity/gitscan/pkg/commands/artifact"
	cr "github.com/aquasecurity/gitscan/pkg/compliance/report"
	"github.com/aquasecurity/gitscan/pkg/flag"
	"github.com/aquasecurity/gitscan/pkg/k8s/report"
	"github.com/aquasecurity/gitscan/pkg/k8s/scanner"
...
//
// e.g.
// $ gitscan k8s --report all cluster
// $ gitscan k8s --report all all
//
// Or they can use "--format json" with implicit "--report all".
//
// e.g. $ gitscan k8s --format json cluster // All the results are shown in JSON
//
// Single resource scanning is allowed with implicit "--report all".
//
// e.g. $ gitscan k8s pod myapp
func validateReportArguments(opts flag.Options) error {
	if opts.ReportFormat == "all" &&
		!viper.IsSet("report") &&
		opts.Format == "table" {

		m := "All the results in the table format can mess up your terminal. Use \"--report all\" to tell GitScan to output it to your terminal anyway, or consider \"--report summary\" to show the summary output."
