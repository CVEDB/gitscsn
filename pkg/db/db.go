...
import (
	...
	"github.com/aquasecurity/gitscan/pkg/oci"
)

const (
	dbMediaType         = "application/vnd.aquasec.gitscan.db.layer.v1.tar+gzip"
...
func (c *Client) NeedsUpdate(cliVersion string, skip bool) (bool, error) {
	...
	if db.SchemaVersion < meta.Version {
		log.Logger.Errorf("GitScan version (%s) is old. Update to the latest version.", cliVersion)
...
func (c *Client) validate(meta metadata.Metadata) error {
	if db.SchemaVersion != meta.Version {
		log.Logger.Error("The local DB has an old schema version which is not supported by the current version of GitScan CLI. DB needs to be updated.")
		...
		if errors.As(err, &terr) {
			for _, diagnostic := range terr.Errors {
				...
				if diagnostic.Code == transport.DeniedErrorCode || diagnostic.Code == transport.UnauthorizedErrorCode {
					log.Logger.Warn("See https://cvedb.github.io/gitscan/latest/docs/references/troubleshooting/#db")
					break
				}
			}
		}
