...
const (
	...
	DefaultIgnoreFile = ".gitscanignore"
)
...
func applyPolicy(ctx context.Context, vulns []types.DetectedVulnerability, misconfs []types.DetectedMisconfiguration,
	...
	query, err := rego.New(
		rego.Query("data.gitscan.ignore"),
		rego.Module("lib.rego", module),
		rego.Module("gitscan.rego", string(policy)),
	).PrepareForEval(ctx)
	if err != nil {
...
func getIgnoredIDs(ignoreFile string) []string {
	...
	if err != nil {
		// gitscan must work even if no .gitscanignore exist
		return nil
	}
	...
	for scanner.Scan() {
		...
		if len(fields) > 1 {
			...
			if err != nil {
				log.Logger.Warnf("Error while parsing expiration date in .gitscanignore file: %s", err)
				continue
			}
			if !exp.IsZero() {
				now := time.Now()
