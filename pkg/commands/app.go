...
import (
	...
	awscommands "github.com/aquasecurity/gitscan/pkg/cloud/aws/commands"
	"github.com/aquasecurity/gitscan/pkg/commands/artifact"
	"github.com/aquasecurity/gitscan/pkg/commands/server"
	"github.com/aquasecurity/gitscan/pkg/fanal/analyzer"
	"github.com/aquasecurity/gitscan/pkg/flag"
	k8scommands "github.com/aquasecurity/gitscan/pkg/k8s/commands"
	"github.com/aquasecurity/gitscan/pkg/log"
	"github.com/aquasecurity/gitscan/pkg/module"
	"github.com/aquasecurity/gitscan/pkg/plugin"
	"github.com/aquasecurity/gitscan/pkg/types"
)

// VersionInfo holds the gitscan DB version Info
...
// NewApp is the factory method to return GitScan CLI
...
func NewRootCommand(version string, globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "gitscan [global flags] command [flags] target",
  ...
  $ gitscan server`,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			// Set the GitScan version here so that we can override version printer.
			cmd.Version = version
...
func NewImageCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ gitscan image --severity HIGH,CRITICAL alpine:3.15

  # Ignore unfixed/unpatched vulnerabilities
  $ gitscan image --ignore-unfixed alpine:3.15

  # Scan a container image in client mode
  $ gitscan image --server http://127.0.0.1:4954 alpine:latest

  # Generate json result
  $ gitscan image --format json --output result.json alpine:3.15

  # Generate a report in the CycloneDX format
  $ gitscan image --format cyclonedx --output result.cdx alpine:3.15`,
...
func NewFilesystemCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  # Scan a single file
  $ gitscan fs ./gitscan-ci-test/Pipfile.lock`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewRepositoryCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ gitscan repo https://github.com/knqyf263/gitscan-ci-test`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewClientCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			log.Logger.Warn("'client' subcommand is deprecated now. See https://github.com/aquasecurity/gitscan/discussions/2119")
...
func NewServerCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ gitscan server --listen 0.0.0.0:10000
`,
...
func NewKubernetesCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  # resources scanning:
  $ gitscan k8s --report=summary deploy
  $ gitscan k8s --namespace=kube-system --report=summary deploy,configmaps

  # resource scanning:
  $ gitscan k8s deployment/orion
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewAWSCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`Scan an AWS account for misconfigurations. GitScan uses the same authentication methods as the AWS CLI. See https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html
...
`, strings.Join(services, "\n- ")),
  ...
  $ gitscan aws --region us-east-1 --service s3

  # limit scan to multiple services:
  $ gitscan aws --region us-east-1 --service s3 --service ec2

  # force refresh of cache for fresh results
  $ gitscan aws --region us-east-1 --update-cache
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewVMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ gitscan vm --scanners vuln ami:${your_ami_id}

  # Scan your AWS EBS snapshot
  $ gitscan vm ebs:${your_ebs_snapshot_id}
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
...
func NewSBOMCommand(globalFlags *flag.GlobalFlagGroup) *cobra.Command {
  ...
  $ gitscan sbom /path/to/report.cdx

  # Scan CycloneDX and generate a CycloneDX report
  $ gitscan sbom --format cyclonedx /path/to/report.cdx

  # Scan CycloneDX-type attestation and show the result in tables
  $ gitscan sbom /path/to/report.cdx.intoto.jsonl
`,
		PreRunE: func(cmd *cobra.Command, args []string) error {
