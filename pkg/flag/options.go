...
type Options struct {
	...
	// GitScan's version, not populated via CLI flags
	AppVersion string
...
func bind(cmd *cobra.Command, flag *Flag) error {
	if flag == nil {
		...
		// This flag is available only in gitscan.yaml
...
func bindEnv(flag *Flag) error {
	...
	envName := strings.ToUpper("gitscan_" + strings.ReplaceAll(flag.Name, "-", "_"))
	...
	for _, alias := range flag.Aliases {
		envAlias := strings.ToUpper("gitscan_" + strings.ReplaceAll(alias.Name, "-", "_"))
...
func getValue(flag *Flag) any {
	...
	// First, looks for aliases in config file (gitscan.yaml).
