...
const (
	...
	builtinRulesUrl = "https://github.com/aquasecurity/gitscan/blob/main/pkg/fanal/secret/builtin-rules.go" // list all secrets
)
...
func (sw SarifWriter) Write(report types.Report) error {
	...
	sw.run = sarif.NewRunWithInformationURI("GitScan", "https://github.com/aquasecurity/gitscan")
	sw.run.Tool.Driver.WithVersion(sw.Version)
	sw.run.Tool.Driver.WithFullName("GitScan Vulnerability Scanner")
