...
func (w Writer) Write(report types.Report) error {
	...
	snapshot.Detector = Detector{
		Name:    "gitscan",
		Version: w.Version,
		Url:     "https://github.com/aquasecurity/gitscan",
	}
...
func getMetadata(report types.Report) Metadata {
	...
	if report.Metadata.RepoTags != nil {
		metadata["aquasecurity:gitscan:RepoTag"] = strings.Join(report.Metadata.RepoTags, ", ")
	}
	if report.Metadata.RepoDigests != nil {
		metadata["aquasecurity:gitscan:RepoDigest"] = strings.Join(report.Metadata.RepoDigests, ", ")
	}
	return metadata
}
