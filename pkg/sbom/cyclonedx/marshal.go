...
import (
	...
	"github.com/aquasecurity/gitscan/pkg/purl"
	"github.com/aquasecurity/gitscan/pkg/scanner/utils"
...
type Marshaler struct {
	appVersion string // GitScan version
	clock      clock.Clock
	newUUID    newUUID
}
...
// Marshal converts the GitScan report to the CycloneDX format
...
// MarshalVulnerabilities converts the GitScan report to the CycloneDX format only with vulnerabilities.
...
func cdxRatingV2(sourceID dtypes.SourceID, severity dtypes.Severity, cvss dtypes.CVSS) cdx.VulnerabilityRating {
	...
	// GitScan keeps only CVSSv3 severity for NVD.
