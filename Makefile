...
.PHONY: build
build:
	go build $(LDFLAGS) ./cmd/gitscan

.PHONY: protoc
protoc:
	docker build -t gitscan-protoc - < Dockerfile.protoc
	docker run --rm -it -v ${PWD}:/app -w /app gitscan-protoc make _$@
...
.PHONY: install
install:
	go install $(LDFLAGS) ./cmd/gitscan
...
.PHONY: label
label: $(GOBIN)/labeler
	labeler apply misc/triage/labels.yaml -r aquasecurity/gitscan -l 5
