...
GitLab 15.0 includes [free](https://gitlab.com/groups/gitlab-org/-/epics/2233) integration with GitScan.

To [configure container scanning with GitScan in GitLab](https://docs.gitlab.com/ee/user/application_security/container_scanning/#configuration), simply include the CI template in your `.gitlab-ci.yml` file:
...
gitscan:
  ...
  variables:
    ...
    DOCKER_TLS_CERTDIR: ""
    IMAGE: gitscan-ci-test:$CI_COMMIT_SHA
    TRIVY_NO_PROGRESS: "true"
    TRIVY_CACHE_DIR: ".gitscancache/"
  before_script:
    - export TRIVY_VERSION=$(wget -qO - "https://api.github.com/repos/aquasecurity/gitscan/releases/latest" | grep '"tag_name":' | sed -E 's/.*"v([^"]+)".*/\1/')
    - echo $TRIVY_VERSION
    - wget --no-verbose https://github.com/aquasecurity/gitscan/releases/download/v${TRIVY_VERSION}/gitscan_${TRIVY_VERSION}_Linux-64bit.tar.gz -O - | tar -zxvf -
  allow_failure: true
  script:
    # Build image
    - docker build -t $IMAGE .
    # Build report
    - ./gitscan image --exit-code 0 --format template --template "@contrib/gitlab.tpl" -o gl-container-scanning-report.json $IMAGE
    # Print report
    - ./gitscan image --exit-code 0 --severity HIGH $IMAGE
    # Fail on severe vulnerabilities
    - ./gitscan image --exit-code 1 --severity CRITICAL $IMAGE
  cache:
    paths:
      - .gitscancache/
...
### GitLab CI using GitScan container
...
In case of a non-public GitLab project GitScan additionally needs to
...
container_scanning:
  image:
    name: docker.io/aquasec/gitscan:latest
    entrypoint: [""]
  variables:
  ...
  script:
    ...
    - time gitscan image --clear-cache
    # update vulnerabilities db
    - time gitscan image --download-db-only
    # Builds report and puts it in the default workdir $CI_PROJECT_DIR, so `artifacts:` can take it from there
    - time gitscan image --exit-code 0 --format template --template "@/contrib/gitlab.tpl"
        --output "$CI_PROJECT_DIR/gl-container-scanning-report.json" "$FULL_IMAGE_NAME"
    # Prints full report
    - time gitscan image --exit-code 0 "$FULL_IMAGE_NAME"
    # Fail on critical vulnerabilities
    - time gitscan image --exit-code 1 --severity CRITICAL "$FULL_IMAGE_NAME"
  cache:
    paths:
      - .gitscancache/
  # Enables https://docs.gitlab.com/ee/user/application_security/container_scanning/ (Container Scanning report is available on GitLab EE Ultimate or GitLab.com Gold)
...
[example]: https://gitlab.com/aquasecurity/gitscan-ci-test/pipelines
...
gitscan:
  ...
  script:
    ...
    - docker build -t $IMAGE .
    # Image report
    - ./gitscan image --exit-code 0 --format template --template "@contrib/gitlab-codequality.tpl" -o gl-codeclimate-image.json $IMAGE
    # Filesystem report
    - ./gitscan filesystem --scanners config,vuln --exit-code 0 --format template --template "@contrib/gitlab-codequality.tpl" -o gl-codeclimate-fs.json .
    # Combine report
...
combine the previous artifact with the output of gitscan, the following `jq`
command can be used, `jq -s 'add' prev-codeclimate.json gitscan-codeclimate.json > gl-codeclimate.json`.
