...
GitScan provides built-in policies to detect configuration issues in popular Infrastructure as Code files, such as: Docker, Kubernetes, Terraform, CloudFormation, and more. 
...
``` bash
$ gitscan config [YOUR_IaC_DIRECTORY]
```
...
```bash
$ gitscan image --scanners config IMAGE_NAME
```

```bash
$ gitscan fs --scanners config /path/to/dir
```
...
!!! example
    ...
    $ gitscan fs --scanners vuln,config,secret --severity HIGH,CRITICAL myapp/
...
In the above example, GitScan detected vulnerabilities of Python dependencies and misconfigurations in Dockerfile.

## Type detection
The specified directory can contain mixed types of IaC files.
GitScan automatically detects config types and applies relevant policies.
...
$ gitscan conf --severity HIGH,CRITICAL ./iac
```
...
## Examples
See [here](https://github.com/aquasecurity/gitscan/tree/{{ git.tag }}/examples/misconf/mixed)
