...
When misconfiguration detection is enabled, GitScan pulls the OPA bundle from GHCR as an OCI artifact and stores it in the cache.
Those policies are then loaded into GitScan OPA engine and used for detecting misconfigurations.
If GitScan is unable to pull down newer policies, it will use the embedded set of policies as a fallback. This is also the case in air-gap environments where `--skip-policy-update` might be passed.

## Update Interval
GitScan checks for updates to OPA bundle on GHCR every 24 hours and pulls it if there are any updates.
