...
## Container images
GitScan can retrieve SBOM attestation of the specified container image in the [Rekor][rekor] instance and scan it for vulnerabilities.
...
### Scanning
You need to pass `--sbom-sources rekor` so that GitScan will look for SBOM attestation in Rekor.

!!! note
    `--sbom-sources` can be used only with `gitscan image` at the moment.

```bash
$ gitscan image --sbom-sources rekor otms61/alpine:3.7.3                                                                            [~/src/github.com/aquasecurity/gitscan]
...
2022-09-16T17:37:13.258+0900	INFO	Please see also https://cvedb.github.io/gitscan/dev/docs/secret/scanning/#recommendation for faster secret detection
...
```bash
$ gitscan image --sbom-sources rekor --rekor-url https://my-rekor.dev otms61/alpine:3.7.3
```

## Non-packaged binaries
GitScan can retrieve SBOM attestation of non-packaged binaries in the [Rekor][rekor] instance and scan it for vulnerabilities.
...
$ gitscan fs --format cyclonedx --output bat.cdx ./bat/Cargo.lock
```
...
$ gitscan plugin install github.com/aquasecurity/gitscan-plugin-attest
$ gitscan attest --predicate ./bat.cdx --type cyclonedx ./bat-v0.20.0-x86_64-apple-darwin/bat
```
...
GitScan calculates the digest of the `bat` binary and searches for the SBOM attestation by the digest in Rekor.
If it is found, GitScan uses that for vulnerability scanning.

```bash
$ gitscan fs --sbom-sources rekor ./bat-v0.20.0-x86_64-apple-darwin/bat
...
```bash
$ gitscan image --sbom-sources rekor --scanners vuln alpine-with-bat
...
[plugin-attest]: https://github.com/aquasecurity/gitscan-plugin-attest
