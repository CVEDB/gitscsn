...
And, GitScan can take an SBOM attestation as input and scan for vulnerabilities
...
In the following example, GitScan generates an SBOM in the CycloneDX format, and then Cosign attaches an attestation of the SBOM to a container image with a local key pair.

```bash
# The cyclonedx type is supported in Cosign v1.10.0 or later.
$ gitscan image --format cyclonedx -o sbom.cdx.json <IMAGE>
...
```bash
# spdx
$ gitscan image --format spdx -o sbom.spdx <IMAGE>
$ cosign attest --key /path/to/cosign.key --type spdx --predicate sbom.spdx <IMAGE>

# spdx-json
$ gitscan image --format spdx-json -o sbom.spdx.json <IMAGE>
...
GitScan can take an SBOM attestation as input and scan for vulnerabilities. Currently, GitScan supports CycloneDX-type attestation.

In the following example, Cosign can get an CycloneDX-type attestation and gitscan scan it.
...
$ gitscan sbom ./sbom.cdx.intoto.jsonl
