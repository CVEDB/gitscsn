...
GitScan generates reports in the [Cosign vulnerability scan record format][vuln-attest-spec].

You can use the regular subcommands (like image, fs and rootfs) and specify `cosign-vuln` with the --format option.

```
$ gitscan image --format cosign-vuln --output vuln.json alpine:3.10
```
...
```json
{
  ...
  "scanner": {
    "uri": "pkg:github/aquasecurity/gitscan@v0.30.1-8-gf9cb8a28",
...
In the following example, GitScan generates a cosign vulnerability scan record, and then Cosign attaches an attestation of it to a container image with a local key pair.

```
$ gitscan image --format cosign-vuln --output vuln.json <IMAGE>
...
```
$ gitscan image --format cosign-vuln -o vuln.json <IMAGE>
