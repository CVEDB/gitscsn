...
```bash
$ gitscan vm --scanners vuln disk.vmdk
```
...
```shell
$ gitscan vm --scanners vuln ami:ami-0123456789abcdefg
```

If you want to scan a AMI of non-default setting region, you can set any region via `--aws-region` option.

```shell
$ gitscan vm --aws-region ap-northeast-1 ami:ami-0123456789abcdefg
```


#### Required Actions
Some actions on EBS are also necessary since GitScan scans an EBS snapshot tied to the specified AMI under the hood.
...
```shell
$ gitscan vm --scanners vuln ebs:snap-0123456789abcdefg
```
...
```shell
$ gitscan vm --aws-region ap-northeast-1 ebs:ebs-0123456789abcdefg
```
...
Then, GitScan can scan the local VM image file.
...
## Scanners
GitScan supports VM image scanning for
...
```
$ gitscan vm --scanners config [YOUR_VM_IMAGE]
```
...
!!! tip
    The scanning could be faster if you enable only vulnerability scanning (`--scanners vuln`) because GitScan tries to download only necessary blocks for vulnerability detection.
...
```shell
$ gitscan vm --scanners license [YOUR_VM_IMAGE]
```

## SBOM generation
GitScan can generate SBOM for VM images.
