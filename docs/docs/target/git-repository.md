...
```bash
$ gitscan repo [YOUR_REPO_URL]
```

## Scanners
### Vulnerabilities
It is enabled by default.
GitScan will look for vulnerabilities based on lock files such as Gemfile.lock and package-lock.json.
...
```shell
$ gitscan repo --scanners config [YOUR_REPO_URL]
```
...
```shell
$ gitscan repo --scanners license [YOUR_REPO_URL]
```

## SBOM generation
GitScan can generate SBOM for git repositories.
...
```
$ gitscan repo --branch <branch-name> <repo-name>
```
...
```
$ gitscan repo --commit <commit-hash> <repo-name>
```
...
```
$ gitscan repo --tag <tag-name> <repo-name>
```
...
$ gitscan repo <your private GitHub repo URL>
$
$ # or
$ export GITLAB_TOKEN="your_private_gitlab_token"
$ gitscan repo <your private GitLab repo URL>
```
