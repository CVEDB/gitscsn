...
GitScan supports two targets for container images.
...
GitScan scans the files inside container images for
...
```shell
$ gitscan image --scanners vuln [YOUR_IMAGE_NAME]
```

### Misconfigurations
It is supported, but it is not useful in most cases.
As mentioned [here](../misconfiguration/scanning.md), GitScan mainly supports Infrastructure as Code (IaC) files for misconfigurations.
...
```
$ gitscan image --scanners config [YOUR_IMAGE_NAME]
```
...
```shell
$ gitscan image --scanners license [YOUR_IMAGE_NAME]
```
...
GitScan scans the configuration of container images for
...
### Misconfigurations
GitScan detects misconfigurations on the configuration of container images.
The image config is converted into Dockerfile and GitScan handles it as Dockerfile.
...
```
$ gitscan image --image-config-scanners config [YOUR_IMAGE_NAME]
```
...
```
$ gitscan image --scanners none --image-config-scanners config alpine:3.17.0
```
...
### Secrets
GitScan detects secrets on the configuration of container images.
The image config is converted into JSON and GitScan scans the file for secrets.
It is especially useful for environment variables that are likely to have credentials by accident.
See [here](../secret/scanning.md) for the detail.

```shell
$ gitscan image --image-config-scanners secret [YOUR_IMAGE_NAME]
```
...
```shell
$ gitscan image --scanners none --image-config-scanners secret vuln-image
```
...
## Supported
### Docker Engine
GitScan tries to looks for the specified image in your local Docker Engine.
...
GitScan tries to looks for the specified image in your local [containerd](https://containerd.io/).
...
Before performing GitScan commands, you must enable the podman.sock systemd service on your machine.
...
### Container Registry
GitScan supports registries that comply with the following specifications.
...
### Tar Files
GitScan supports image tar files generated by the following tools.
...
### OCI Layout
GitScan supports image directories compliant with [Open Container Image Layout Specification](https://github.com/opencontainers/image-spec/blob/master/spec.md).

Buildah:

```
$ buildah push docker.io/library/alpine:3.11 oci:/path/to/alpine
$ gitscan image --input /path/to/alpine
```
...
## SBOM generation
GitScan can generate SBOM for container images.
...
```
$ gitscan image --compliance docker-cis [YOUR_IMAGE_NAME]
```
...
By default, GitScan loads an image on a "linux/amd64" machine.
To customise this, pass a `--platform` argument in the format OS/Architecture for the image:

```
$ gitscan image --platform=os/architecture [YOUR_IMAGE_NAME]
```

For example:

```
$ gitscan image --platform=linux/arm alpine:3.16.1
```
...
2022-10-25T21:00:50.972+0300    INFO    Please see also https://cvedb.github.io/gitscan/dev/docs/secret/scanning/#recommendation for faster secret detection
