...
```bash
$ gitscan fs /path/to/project
```

It's also possible to scan a single file.

```
$ gitscan fs ~/src/github.com/aquasecurity/gitscan-ci-test/Pipfile.lock
```
...
```
$ gitscan fs ~/src/github.com/aquasecurity/gitscan-ci-test
```
...
```shell
$ gitscan fs --scanners config /path/to/project
```
...
```shell
$ gitscan fs --scanners license /path/to/project
```

## SBOM generation
GitScan can generate SBOM for local projects.
