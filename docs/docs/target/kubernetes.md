...
## CLI
The GitScan K8s CLI allows you to scan your Kubernetes cluster for 
...
The difference to the GitScan CLI is that the GitScan K8s CLI allows you to scan running workloads directly within your cluster.

If you are looking for continuous cluster audit scanning, have a look at the GitScan K8s operator below.

GitScan uses your local kubectl configuration to access the API server to list artifacts.
...
![k8s Summary Report](../../imgs/gitscan-k8s.png)
...
```
$ gitscan k8s --severity=CRITICAL --report=all cluster
```

Filter by scanners (Vulnerabilities, Secrets or Misconfigurations):

```
$ gitscan k8s --scanners=secret --report=summary cluster
# or
$ gitscan k8s --scanners=config --report=summary cluster
```

Scan a specific namespace:

```
$ gitscan k8s -n kube-system --report=summary all
```

Use a specific kubeconfig file:

```
$ gitscan k8s --kubeconfig ~/.kube/config2 -n kube-system --report=summary all
```

Scan a specific resource and get all the output:

```
$ gitscan k8s deployment appname
```

Scan all deploys, or deploys and configmaps:

```
$ gitscan k8s --report=summary deployment
$ gitscan k8s --report=summary deployment,configmaps
```
...
```
$ gitscan k8s -n app --severity=CRITICAL deployment/appname
```
This is specific to all GitScan CLI commands.

The supported formats are `table`, which is the default, and `json`.
To get a JSON output on a full cluster scan:

```
$ gitscan k8s --format json -o results.json cluster
```
...
GitScan by default scans kubernetes infra components (apiserver, controller-manager, scheduler and etcd)
...
```
$ gitscan k8s cluster --report summary # full cluster scan
$ gitscan k8s all -n kube-system --report summary # scan all components under kube-system
```
...
```
$ gitscan k8s cluster --report summary --components=infra --scanners=config # scan only infra
```

Or, to filter for all other checks besides the infra checks, you can:

```
$ gitscan k8s cluster --report summary --components=workload --scanners=config # scan all components besides infra
```
...
```
$ gitscan k8s cluster --compliance=<compliance_id> --report summary
```
...
```
gitscan k8s cluster --compliance=<compliance_id> --report all
```

Report result in JSON format:

```
gitscan k8s cluster --compliance=<compliance_id> --report summary --format json
```

```
gitscan k8s cluster --compliance=<compliance_id> --report all --format json
```

## Operator
GitScan has a native [Kubernetes Operator][operator] which continuously scans your Kubernetes cluster for security issues, and generates security reports as Kubernetes [Custom Resources][crd]. It does it by watching Kubernetes for state changes and automatically triggering scans in response to changes, for example initiating a vulnerability scan when a new Pod is created.

> Kubernetes-native security toolkit. ([Documentation][gitscan-operator]).
...
[gitscan-operator]: https://cvedb.github.io/gitscan-operator/latest
