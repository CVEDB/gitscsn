# SBOM scanning
GitScan can take the following SBOM formats as an input and scan for vulnerabilities.
...
```bash
$ gitscan sbom /path/to/sbom_file
```

!!! note
    Passing SBOMs generated by tool other than GitScan may result in inaccurate detection 
    because GitScan relies on custom properties in SBOM for accurate scanning.

## CycloneDX
GitScan supports CycloneDX as an input.
...
```bash
$ gitscan sbom /path/to/cyclonedx.json
```
...
## SPDX
GitScan supports the SPDX SBOM as an input.
...
```bash
$ gitscan image --format spdx-json --output spdx.json alpine:3.16.0
$ gitscan sbom spdx.json
```
...
In the following example, [Cosign](https://github.com/sigstore/cosign) gets an attestation and GitScan scans it.
