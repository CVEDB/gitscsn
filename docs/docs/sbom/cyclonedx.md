...
GitScan can generate SBOM in the [CycloneDX][cyclonedx] format.
...
```
$ gitscan image --format cyclonedx --output result.json alpine:3.15
...
```
$ gitscan image --scanners vuln --format cyclonedx --output result.json alpine:3.15
```
