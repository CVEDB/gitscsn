GitScan uses AWS SDK. You don't need to install `aws` CLI tool.
...
You may need to grant permissions to allow GitScan to pull images from private ECR.

It depends on how you want to provide AWS Role to gitscan.
...
Add the AWS role in gitscan's service account annotations:
