...
GitScan provides a module feature to allow others to extend the GitScan CLI without the need to change the GitScan code base.
It changes the behavior during scanning by WebAssembly.

## Overview
GitScan modules are add-on tools that integrate seamlessly with GitScan.
They provide a way to extend the core feature set of GitScan, but without updating the GitScan binary.
...
!!! warning
    ...
    WebAssembly is sandboxed and secure by design, but GitScan modules available in public are not audited for security.
    You should install and run third-party modules at your own risk even though 

Under the hood GitScan leverages [wazero][wazero] to run WebAssembly modules without CGO.

## Installing a Module
A module can be installed using the `gitscan module install` command.
...
GitScan will now search XDG_DATA_HOME for the location of the GitScan modules cache.
The preference order is as follows:

- XDG_DATA_HOME if set and .gitscan/plugins exists within the XDG_DATA_HOME dir
- $HOME/.gitscan/plugins
...
## Using Modules
Once the module is installed, GitScan will load all available modules in the cache on the start of the next GitScan execution.
The modules may inject custom logic into scanning and change the result.
You can run GitScan as usual and modules are loaded automatically.

You will see the log messages about WASM modules.

```shell
$ gitscan image ghcr.io/aquasecurity/gitscan-test-images:spring4shell-jre8
2022-06-12T12:57:13.210+0300    INFO    Loading ghcr.io/aquasecurity/gitscan-module-spring4shell/spring4shell.wasm...
...
Specify a module repository with `gitscan module uninstall` command.

```bash
$ gitscan module uninstall ghcr.io/aquasecurity/gitscan-module-spring4shell
```
...
### TinyGo
GitScan provides Go SDK including three interfaces.
...
```
$ go mod init github.com/aquasecurity/gitscan-module-wordpress
```
...
!!! tips
    GitScan caches analysis results according to the module version.
    We'd recommend cleaning the cache or changing the module version every time you update `Analyzer`.


#### PostScanner interface
`PostScan` is called after scanning and takes the scan result as an argument from GitScan.
...
If you are interested in `Update`, you can see an example of [Spring4Shell][gitscan-module-spring4shell].

In the `Delete` action, `PostScan` needs to return results you want to delete.
If `PostScan` returns an empty, GitScan will not delete anything.
...
```bash
oras push ghcr.io/aquasecurity/gitscan-module-wordpress:latest wordpress.wasm:application/vnd.module.wasm.content.layer.v1+wasm
Uploading 3daa3dac086b wordpress.wasm
Pushed ghcr.io/aquasecurity/gitscan-module-wordpress:latest
Digest: sha256:6416d0199d66ce52ced19f01d75454b22692ff3aa7737e45f7a189880840424f
```

## Examples
- [Spring4Shell][gitscan-module-spring4shell]
- [WordPress][gitscan-module-wordpress]
...
[gitscan-module-spring4shell]: https://github.com/aquasecurity/gitscan/tree/main/examples/module/spring4shell
[gitscan-module-wordpress]: https://github.com/aquasecurity/gitscan-module-wordpress
