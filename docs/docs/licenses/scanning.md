...
GitScan scans any container image for license files and offers an opinionated view on the risk associated with the license.
...
!!! tip
    Licenses that GitScan fails to recognize are classified as UNKNOWN.
    As those licenses may be in violation, it is recommended to check those unknown licenses as well.    

By default, GitScan scans licenses for packages installed by `apk`, `apt-get`, `dnf`, `npm`, `pip`, `gem`, etc.
To enable extended license scanning, you can use `--license-full`.
In addition to package licenses, GitScan scans source code files, Markdown documents, text files and `LICENSE` documents to identify license usage within the image or filesystem.
...
``` shell
$ gitscan image --scanners license --severity UNKNOWN,HIGH,CRITICAL alpine:3.15
...
``` shell
$ gitscan image --scanners license --severity UNKNOWN,HIGH,CRITICAL --license-full grafana/grafana
...
GitScan has number of configuration flags for use with license scanning;
                                 
### Ignored Licenses

GitScan license scanning can ignore licenses that are identified to explicitly remove them from the results using the `--ignored-licenses` flag;

```shell
$ gitscan image --scanners license --ignored-licenses MPL-2.0,MIT --severity LOW grafana/grafana:latest
...
```shell
$ gitscan image --generate-default-config
$ vim gitscan.yaml
license:
  forbidden:
  - AGPL-3.0
