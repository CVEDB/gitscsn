...
GitScan can be customized by tweaking a `gitscan.yaml` file. The config path can be overridden by the `--config` flag.
...
# Same as '--report' (available with 'gitscan k8s')
# Default is all
report: all
...
# Same as '--ignorefile'
# Default is '.gitscanignore'
ignorefile: .gitscanignore
...
db:
  ...
  # Default is 'ghcr.io/aquasecurity/gitscan-java-db'
  java-repository: ghcr.io/aquasecurity/gitscan-java-db
```
...
```yaml
image:
  # Same as '--input' (available with 'gitscan image')
  # Default is empty
  input:
...
secret:
  ...
  # Default is 'gitscan-secret.yaml'
  config: config/gitscan/secret.yaml
```
...
## Repository Options
Available with git repository scanning (`gitscan repo`)
...
Available for cloud scanning (currently only `gitscan aws`)
...
[example]: https://github.com/aquasecurity/gitscan/tree/{{ git.tag }}/examples/gitscan-conf/gitscan.yaml
