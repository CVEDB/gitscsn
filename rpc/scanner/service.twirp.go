...
func NewScannerProtobufClient(baseURL string, client HTTPClient, opts ...twirp.ClientOption) Scanner {
	...
	serviceURL += baseServicePath(pathPrefix, "gitscan.scanner.v1", "Scanner")
	urls := [1]string{
		serviceURL + "Scan",
	}
...
func (c *scannerProtobufClient) Scan(ctx context.Context, in *ScanRequest) (*ScanResponse, error) {
	ctx = ctxsetters.WithPackageName(ctx, "gitscan.scanner.v1")
...
const ScannerPathPrefix = "/twirp/gitscan.scanner.v1.Scanner/"
...
func (s *scannerServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	if pkgService != "gitscan.scanner.v1.Scanner" {
...
func (s *scannerServer) PathPrefix() string {
	return baseServicePath(s.pathPrefix, "gitscan.scanner.v1", "Scanner")
}
