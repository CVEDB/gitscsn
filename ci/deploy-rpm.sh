...
function create_rpm_repo () {
        ...
        createrepo_c -u https://github.com/aquasecurity/gitscan/releases/download/ --location-prefix="v"$TRIVY_VERSION --update $rpm_path

        rm ${rpm_path}/*64bit.rpm
}

echo "Create RPM releases for GitScan v$TRIVY_VERSION"
...
git add .
git commit -m "Update rpm packages for GitScan v$TRIVY_VERSION"
