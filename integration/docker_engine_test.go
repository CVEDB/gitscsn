...
func TestDockerEngine(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "alpine:3.9",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:alpine-39",
		...
		{
			name:      "alpine:3.9, with .gitscanignore",
			imageTag:  "ghcr.io/aquasecurity/gitscan-test-images:alpine-39",
		...
		},
		{
			...
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:ubi-7",
			input:    "testdata/fixtures/images/ubi-7.tar.gz",
			golden:   "testdata/ubi-7.json.golden",
		},
		{
			name:     "debian buster/10",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:debian-buster",
		...
		},
		{
			...
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:debian-stretch",
			input:    "testdata/fixtures/images/debian-stretch.tar.gz",
			golden:   "testdata/debian-stretch.json.golden",
		},
		{
			name:     "distroless base",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:distroless-base",
		...
		},
		{
			...
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:opensuse-leap-151",
			input:    "testdata/fixtures/images/opensuse-leap-151.tar.gz",
			golden:   "testdata/opensuse-leap-151.json.golden",
		},
		{
			name:     "photon 3.0",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:photon-30",
			input:    "testdata/fixtures/images/photon-30.tar.gz",
			golden:   "testdata/photon-30.json.golden",
		},
		{
			name:     "CBL-Mariner 1.0",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:mariner-1.0",
			input:    "testdata/fixtures/images/mariner-1.0.tar.gz",
			golden:   "testdata/mariner-1.0.json.golden",
		},
		{
			name:     "busybox with Cargo.lock",
			imageTag: "ghcr.io/aquasecurity/gitscan-test-images:busybox-with-lockfile",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(tt.ignoreIDs) != 0 {
				gitscanIgnore := ".gitscanignore"
				err = os.WriteFile(gitscanIgnore, []byte(strings.Join(tt.ignoreIDs, "\n")), 0444)
				assert.NoError(t, err, "failed to write .gitscanignore")
				defer os.Remove(gitscanIgnore)
			}
