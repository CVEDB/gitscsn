...
import (
	...
	"github.com/aquasecurity/gitscan/pkg/clock"
	...
	tests := []struct {
		...
		},
		{
			...
			args: csArgs{
				Command:          "fs",
				RemoteAddrOption: "--server",
				secretConfig:     "testdata/fixtures/fs/secrets/gitscan-secret.yaml",
		...
		},
		{
			...
			args: csArgs{
				Command:          "repo",
				RemoteAddrOption: "--server",
				Target:           "https://github.com/knqyf263/gitscan-ci-test",
			},
...
func TestClientServerWithToken(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			args: csArgs{
				Input:             "testdata/fixtures/images/alpine-39.tar.gz",
				ClientToken:       "token",
				ClientTokenHeader: "GitScan-Token",
			},
			golden: "testdata/alpine-39.json.golden",
		},
		{
	...
	serverToken := "token"
	serverTokenHeader := "GitScan-Token"
	addr, cacheDir := setup(t, setupOptions{
		token:       serverToken,
...
func setupClient(t *testing.T, c csArgs, addr string, cacheDir string, golden string) ([]string, string) {
	...
	if len(c.IgnoreIDs) != 0 {
		gitscanIgnore := filepath.Join(t.TempDir(), ".gitscanignore")
		err := os.WriteFile(gitscanIgnore, []byte(strings.Join(c.IgnoreIDs, "\n")), 0444)
		require.NoError(t, err, "failed to write .gitscanignore")
		osArgs = append(osArgs, "--ignorefile", gitscanIgnore)
	}
	if c.ClientToken != "" {
